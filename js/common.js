 var SITE_URL='http://localhost/devgoeventz/GEsite/';
            	...
            	$.each(data, function(index, element)
                 {
					 if(index=='error')
					 {
						 if(element.email)
						 {
							 $('input[name=email]').addClass("requiredVal");
						 }
						 if(element.password)
						 {
							 $('input[name=password]').addClass("requiredVal");
						 ...
						 {
							 $('.header').append('<div class="alert alert-warning fade in">Email Id Is Already Exist.</div>');
						 }
					 }
					 else if(index == 'loginsuccess')
					 {
						 ...
						 {
							  $(':input').val('');
							  $('.header').append('<div class="alert alert-warning fade in"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Please Try later..</div>');
							 return false;
						 }
					 }
				});
