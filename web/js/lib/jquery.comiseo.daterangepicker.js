...
(function($, window, undefined) {
	...
	textfordisplay='Select date range'; // placeholder text - shown when nothing is selected
	var defaulttext = $('#e2').attr('rel');
	if(defaulttext!='')
	{
		textfordisplay=defaulttext;
	}
...
//				showCurrentAtPos: 1 // bug; use maxDate instead
	...
	function buildButtonPanel(classnameContext, options, handlers) {
		...
		function init() {
			...
			if (options.applyButtonText) {
				applyButton = $('<button type="button" class="ui-priority-primary " id="reportform"></button>')
	...
	function buildDateRangePicker($originalElement, instance, options) {
		...
		function setRange(value, event) {
			...
			triggerButton.setLabel(formatRangeForDisplay(range));
			//alert(formatRangeForDisplay(range));
			//$('#e2').val(formatRangeForDisplay(range)).change();
			//alert($('#e2').val());
			$originalElement.val(formatRangeForDisplay(range));
			if (options.onChange) {
				options.onChange();
			}
